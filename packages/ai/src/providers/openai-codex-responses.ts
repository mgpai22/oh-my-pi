...
async function buildCodexRequestContext(
	...
	const metadataSessionId = normalizeOpenAIPromptCacheKey(options?.sessionId) ?? crypto.randomUUID();
...
export async function prewarmOpenAICodexResponses(
	...
	const metadataSession = getOrCreateCodexMetadataSessionState(
		normalizeOpenAIPromptCacheKey(options?.sessionId) ?? crypto.randomUUID(),
		providerSessionState,
	);
